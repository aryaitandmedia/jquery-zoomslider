...
!(function (s) {
  function t(i, t) {
    ...
    return (
      (this.settings = s.extend({}, n, l, t)),
      null == this.settings.src || this.settings.src.length < 1
  ...
  s.extend(t.prototype, {
    init: function () {
      ...
      e.attr("title", this.settings.title[0]);
    },
    initSlideshow: function () {
      var t = this,
        e = s('<div class="zs-slideshow"></div>'),
        n = s('<div class="zs-slides"></div>'),
        tt = s('<div class="zs-titles"></div>'),
      ...
      for (i = 0; i < this.numSlides; i++) {
        ...
        tl.appendTo(tt),
          0 == i && (tl.addClass("active"), tl.addClass("active"));

        var o = s('<div class="zs-bullet zs-bullet-' + i + '"></div>');
        o.appendTo(a),
          0 == i &&
            (l.addClass("active").css("opacity", 1), o.addClass("active"));
      }
      if (
        (t._promoteChildren(),
        e.append(n).prependTo(this.$el),
        e.append(tt).prependTo(this.$el),
        1 == this.settings.bullets &&
          (e.append(a),
          e.on("click", ".zs-bullet", function () {
            t.jump(s(this).index());
          })),
        (this.pos = 0),
        (this.pending = null),
    ...
    initSingle: function () {
      var i = this,
        ...
        tt = s('<div class="zs-titles"></div>'),
        ttt = s('<div class="zs-title zs-title-0"></div>');
      n
        .css({
          "background-image": "url('" + this.settings.src[0] + "')",
        })
        .appendTo(e),
        ttt.html(this.settings.src[0]).appendTo(tt),
        ttt.addClass("active").css("opacity", 1),
        n.addClass("active").css("opacity", 1),
        i._promoteChildren(),
        t.append(e).prependTo(this.$el),
        t.append(tt).prependTo(this.$el),
        this.$el.addClass("zs-enabled"),
        "dots" == this.settings.overlay
          ? this.$el.addClass("overlay-dots")
          : "plain" == this.settings.overlay &&
    ...
    jump: function (s) {
      ...
      if (this.pos != s) {
        ...
        this.support
            ...
            e
              ...
              .on(this.transEndEventName, function (s) {
                "opacity" == s.originalEvent.propertyName &&
                  ((lastSlideBg = t.css("background-image")),
                  t
                    .removeClass("active")
                    .removeAttr("style")
          ...
          this.$titles.eq(this.pos).removeClass("active"),
          this.$titles.eq(s).addClass("active"),
          (this.pos = s),
    ...
    (s.fn[e] = function (i) {
      return this.each(function () {
        s.data(this, "plugin_" + e) ||
          s.data(this, "plugin_" + e, new t(this, i));
      });
    });
  var a = s("[data-zs-src]");
  a.length > 0 &&
